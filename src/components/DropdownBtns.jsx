      ...
      <div className="w-full max-w-[1632px] flex flex-col gap-[16px] rounded-[5px] rotate-0 opacity-100 p-[48px]">
        {items.map((section, index) => (
          <div key={index} >
            ...
            >
           <div
  className={`group content ${cursorPointer ? 'cursor-pointer' : 'cursor-default'}
    w-[1536px] h-[213px]
    rounded-[16px] mx-auto px-4 sm:px-6 lg:px-[96px]
    border border-[#E5E5E5] transition-colors duration-500 ease-in-out
    flex flex-col justify-center items-center text-center
    ${activeIndex === index ? 'bg-white' : 'hover:bg-[#FAFAF5] bg-white'}`}
>
                ...
                >
                  <span className="capitalize w-full max-w-[306px] h-[36px]">
                    ...
                    {section.title === 'Levels' ? (
                      ...
                      <>
                        <span className="font-tt font-regular group-hover:font-ivy ">{section.title.split('&')[0].toLowerCase()}</span>
                        <span className="font-ivy mx-1">&</span>
                        <span className="font-tt font-regular group-hover:font-ivy">{section.title.split('&')[1].toLowerCase()}</span>
                      </>
                    )}
                  </span>
                </Link>
            ...
            {activeIndex === index && (
              <>
                {(showMoreMap[index] ? section.items : section.items.slice(0, 5)).map((item, idx, arr) => (
                  <div key={item.id} className='gap-[24px]'>
                    <div
                      className={`cursor-default w-[1536px] h-[135px] mt-[64px] pt-[10px] flex flex-col md:flex-row items-center md:items-start justify-between rounded-[16px] z-20 shadomax-w-md ${hasHoveredMap[item.id] ? 'bg-[#FAFAF5]' : 'bg-white'} transition-colors duration-500 ease-in-out`}
                      onMouseEnter={() => {
                    ...
                    >
                      <div className="font-ivy font-regular md:mx-0 text-[24px] sm:text-[28px] md:text-[32px] pt-[25px] pl-[10px] pb-[8px] md:max-w-[260px] h-[43px] leading-[135%] uppercase">
                        {item.title}
                      </div>
                      <div
                        className={`font-tt font-regular text-[18px] sm:text-[20px] md:text-[24px] text-[#404040] leading-[150%] w-full whitespace-normal break-normal transition-all duration-500 ease-in-out ${hasHoveredMap[item.id] ? 'max-w-[835px] h-[87px] translate-x-[-80px]' : 'max-w-[935px] h-[87px] translate-x-0'}`}
                      >
                        {item.description}
                      </div>
                      <div className="relative md:max-w-[200px] h-auto pt-[10px] flex items-center justify-end">
                        <div
                          className={`mr-2 justify-between items-center transition-opacity duration-700 ease-in-out ${hasHoveredMap[item.id] ? 'opacity-100' : 'opacity-0'}`}
                        >
                          <img
                            src="/image/icon3.png"
                            alt="Icon"
                        ...
                        <div
                          className={`absolute right-0 flex text-center items-center transition-all duration-700 ease-in-out ${hasHoveredMap[item.id] ? 'opacity-100 translate-x-[-100px]' : 'opacity-100 translate-x-0'}`}
                        >
                          {section.title === 'Levels' ? (
                              ...
                              <span className="text-[32px] sm:text-[36px] md:text-[40px]">
                                {item.price.split('/')[0].startsWith('$') ? (
                                  <>
                                    ...
                                    <span className="font-tt font-semibold">{item.price.split('/')[0].slice(1)}</span>
                                  </>
                                ) : (
                    ...
                    {idx !== (showMoreMap[index] ? section.items.length : 5) - 1 && (
                      <div className="mt-[24px] mb-[24px] w-full flex justify-center">
                        <div className="w-[1536px] h-[1px] bg-[#E5E5E5] opacity-100" />
                      </div>
                    )}
                  </div>
                ))}
